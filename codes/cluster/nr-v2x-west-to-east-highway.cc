...
main (int argc, char *argv[])
{
  ...
  uint16_t numLanes = 3;
  uint16_t interVehicleDist = 78; //meters
  ...
  uint16_t slMaxTxTransNumPssch = 10;
  ...
  uint8_t mcs = 14;
  uint16_t awarenessRange = 200; //in mt
  ...
  cmd.AddValue ("outputDir",
                ...
                outputDir);
  cmd.AddValue ("awarenessRange",
                "Range in meters used in KPIs calculation",
                awarenessRange);
  cmd.AddValue ("outputDir2",
                "directory where to store simulation results",
                outputDir2);
  cmd.AddValue ("UlMode",
                "Used just for plot purpose",
                UlMode);
  cmd.AddValue ("simTag",
  ...
  v2xKpi.SetRangeForV2xKpis (awarenessRange);
